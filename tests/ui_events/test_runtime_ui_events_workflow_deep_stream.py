...
async def test_workflow_ui_level_step_window_includes_tool_events_with_step_path(tmp_path: Path) -> None:
    ...
    kinds = [seg.kind for seg in p]
    ids = {seg.kind: seg.id for seg in p}
    assert "workflow" in kinds and ids.get("workflow") == "wf.ui"
    assert "step" in kinds and ids.get("step") == "s1"
