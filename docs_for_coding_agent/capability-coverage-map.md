...
| CAP-BRIDGE-003 | 离线注入：Fake backend 仍产出证据链 | `openspec/changes/examples-skills-first-v1/specs/examples-human-apps/spec.md` | `src/agently_skills_runtime/config.py` + `src/agently_skills_runtime/runtime.py` | `examples/apps/* --mode offline`（本变更创建） | `pytest -q tests/test_examples_smoke.py`（本变更创建） | `NodeReportV2.events_path != None` 且可读 |
...
| CAP-SKILLS-001 | skills-first：mention 注入（Scheme2） | `openspec/specs/upstream-bridge/spec.md` | `src/agently_skills_runtime/adapters/agent_adapter.py` | `examples/apps/*`（本变更创建） | `pytest -q`（本仓） | WAL 中 `skill_injected` + `report.activated_skills` |
...
| CAP-SSE-001 | HTTP/SSE：事件流 + 终态 + wal_locator | `openspec/changes/examples-skills-first-v1/specs/examples-human-apps/spec.md` | `examples/apps/sse_gateway_minimal/`（本变更创建） | `examples/apps/sse_gateway_minimal/`（本变更创建） | `pytest -q tests/test_examples_smoke.py -k sse`（本变更创建） | SSE 流包含 `run_started` 与终态（含 wal_locator） |

## 2) 上游内置工具清单（pinned: skills_runtime==0.1.4.post2）
