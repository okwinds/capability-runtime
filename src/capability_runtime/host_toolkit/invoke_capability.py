...
- 遵守上游现实约束：tool handler 为同步函数；当子能力为 async API 时，使用“后台线程 + 独立 event loop”执行。
...
def make_invoke_capability_tool(
    *,
    ...
    spec = ToolSpec(
        ...
        description="\n".join(
            [
                ...
                "约束：tool handler 同步；子调用通过后台线程 + 独立 event loop 执行。",
    ...
    def handler(call: ToolCall, ctx: ToolExecutionContext) -> ToolResult:
        ...
        def _run_child_in_thread() -> Dict[str, Any]:
            """
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                fut = ex.submit(_run_child_in_thread)
                digest = fut.result(timeout=float(timeout_ms) / 1000.0 if timeout_ms else None)
